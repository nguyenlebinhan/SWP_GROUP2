    ...
    <body>
        ...
        <div class="main">
            ...
            <div class="toolbar">
                <div class="d-flex flex-wrap gap-2">
                    <c:if test="${canAddEmploymentContract and contract.status == 'PENDING_APPROVAL'}">
                        <form method="post" action="${pageContext.request.contextPath}/v1/employee/contract/cancel">
                            <input type="hidden" name="contractId" value="${contract.contractId}">
                            <button type="submit" class="btn btn-outline-danger"
                                    onclick="return confirm('Bạn có chắc muốn hủy hợp đồng này?');">
                                <i class="fa-solid fa-ban me-1"></i>Hủy hợp đồng
                            </button>
                        </form>
                    </c:if>
                    <c:if test="${canAddEmploymentContract and not contract.status.isFinalStatus()}">
                ...
                <div class="d-flex gap-2">
                    <c:if test="${not empty contract.contractFilePath}">
                        <a class="btn btn-outline-secondary" href="${pageContext.request.contextPath}/v1/employee/contract/download-signed?contractId=${contract.contractId}">
                            <i class="fa-solid fa-download me-1"></i>Tải hợp đồng
                        </a>
                    </c:if>
                </div>
            </div>
            ...
            <div class="page-card">
                ...
                <div class="detail-grid">
                    ...
                    <div class="detail-item">
                        ...
                        <div class="detail-value note-box">${empty contract.note ? 'Không có ghi chú.' : contract.note}</div>
                    </div>
                </div>

                <c:if test="${not empty contract.previousContractId}">
                    <div class="section-title">Hợp đồng trước đó</div>
                    <div class="detail-grid">
                        <div class="detail-item">
                            <span class="detail-label">Mã hợp đồng trước</span>
                            <div class="detail-value">#${contract.previousContractId}</div>
                        </div>
                        <div class="detail-item">
                            <span class="detail-label">Xem</span>
                            <div class="detail-value">
                                <a href="${pageContext.request.contextPath}/v1/employee/contract/detail?contractId=${contract.previousContractId}" class="btn btn-sm btn-outline-primary">
                ...
                <c:if test="${not empty contract.contractFilePath}">
                    ...
                    <div class="detail-grid">
                        <div class="detail-item">
                            ...
                            <div class="detail-value">${contract.contractFileName}</div>
                        </div>
                        <div class="detail-item">
                            <span class="detail-label">Ngày tải lên</span>
                            <div class="detail-value">
                                <c:choose>
                                    <c:when test="${not empty contract.uploadedAt}">
                                        <fmt:formatDate value="${contract.uploadedAt}" pattern="dd/MM/yyyy HH:mm"/>
                                    </c:when>
            ...
            <div id="editFormSection" style="display:none;">
                ...
                <form method="post" action="${pageContext.request.contextPath}/v1/employee/contract/detail">
                    <input type="hidden" name="contractId" value="${contract.contractId}" />
