...
<head>
    ...
    <style>
        ...
        .summary-grid { display:grid; grid-template-columns:repeat(5,minmax(160px,1fr)); gap:16px; margin-bottom:20px; }
        ...
        .allowance-scroll { max-height:260px; overflow-y:auto; }
    ...
    <c:choose>
        ...
        <c:otherwise>
            ...
            <div class="summary-grid">
                ...
                <div class="summary-card">
                    <div class="summary-label">Bảo hiểm công ty đóng</div>
                    <div class="summary-value blue"><fmt:formatNumber value="${p.employerContribution}" type="number" groupingUsed="true" /> VND</div>
                </div>
            </div>
            ...
            <div class="panel h-100">
                ...
                <div class="table-responsive">
                    <table class="table align-middle">
                        <thead>
                            <tr><th>Khoản mục</th><th>Công thức tính</th><th class="text-end">Số tiền</th></tr>
                        </thead>
                        <tbody>
            ...
            <div class="panel">
                ...
                <div class="table-responsive">
                    <table class="table align-middle">
                        <thead><tr><th>Loại bảo hiểm</th><th class="text-end">NV đóng</th><th class="text-end">DN đóng</th></tr></thead>
                        <tbody>
                            <c:set var="totalEmployerAmount" value="${0}" />
                            <c:forEach var="d" items="${payrollPreview.details}">
                                <c:if test="${d.deduction and d.code ne 'PERSONAL_INCOME_TAX' and d.code ne 'UNPAID_DEDUCTION'}">
                                    <c:set var="totalEmployerAmount" value="${totalEmployerAmount + d.employerAmount}" />
                                    <tr>
                                        <td>
                                            <div><c:out value="${d.name}" /></div>
                                            <div class="small text-muted">
                                                Tổng <fmt:formatNumber value="${d.totalRatePercent}" type="number" maxFractionDigits="2" groupingUsed="false" />%
                                                (NV <fmt:formatNumber value="${d.employeeRatePercent}" type="number" maxFractionDigits="2" groupingUsed="false" />% +
                                                DN <fmt:formatNumber value="${d.employerRatePercent}" type="number" maxFractionDigits="2" groupingUsed="false" />%)
                                                trên <fmt:formatNumber value="${d.base}" type="number" groupingUsed="true" />
                                            </div>
                                        </td>
                                        <td class="text-end fw-bold text-danger">-<fmt:formatNumber value="${d.amount}" type="number" groupingUsed="true" /> VND</td>
                                        <td class="text-end fw-bold text-primary"><fmt:formatNumber value="${d.employerAmount}" type="number" groupingUsed="true" /> VND</td>
                                    </tr>
                                </c:if>
                            </c:forEach>
                            <tr class="fw-bold">
                                ...
                                <td class="text-end text-primary"><fmt:formatNumber value="${totalEmployerAmount}" type="number" groupingUsed="true" /> VND</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
